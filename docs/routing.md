...
| `/user/admin/contacts` | `app/user/admin/contacts/page.tsx` | `AdminLayout` | Operational Growth Funnel — top of the conversion arc. 14 KPI totems + 10-column institutional table (Contact · Company · Type · Band · Strength · Collab · Last email · Gmail labels · Email health · Strategic signal) · URL-driven band + investor-type + quality + sort filters · 50/page server pagination · reads from `relationship_contacts` + labels join. `?selected=<contact_id>` opens a server-rendered side drawer (Header · Conversion status · Institutional context · Suggested next action · Timeline · Peers — read-only) with growth-verb next-actions and a linked-user card when the contact has onboarded. |
| `/user/admin/users` | `app/user/admin/users/page.tsx` | `AdminLayout` | Real platform users — second stage of the conversion arc. 11 KPI totems (state + plan breakdowns) + 11-column table (checkbox + User · Company/Org · Role · Linked contact · Status · Tier · Subscription · Promo · Last seen · Created · Owner) · URL-driven status + plan + linked-only + sort filters · joins `users` ↔ `organizations` ↔ `relationship_contacts` ↔ `subscriptions`. Phase 2.D.6: bulk selection + sticky toolbar (Assign tier · Comp · Expire) with campaign attribution + 7-day expiration ring. |
| `/user/admin/campaigns` | `app/user/admin/campaigns/page.tsx` | `AdminLayout` | Activation Layer (Live · Phase 2.D.4 + 2.D.7) — visual card grid (mobile-first 1→2→3 col) as PRIMARY surface · per-card: name · status pill · 4-metric funnel strip (Active/Converted/Failed/Subs) · owner · channel · conversion target · Manage CTA. Filters + table now collapsed under `<details>` for power-user inspection. Form drawer (Create when `selected=new`, Edit when `selected=<id>`) with archive/restore + last-25 invitation list. |
| `/user/admin/subscriptions` | `app/user/admin/subscriptions/page.tsx` | `AdminLayout` | Monetization Layer (Live · Phase 2.D.4 + 2.D.7) — Stripe/Notion-style pricing card grid (mobile-first 1→2→4 col) as PRIMARY surface · `?product=<id>` opens product edit drawer · `?product=new` opens create-product drawer (catalogue is operator-managed data, no longer enum-locked). KPIs strip (status + tier breakdown). Subscribers table relegated to secondary section below: Assign tier · Update tier/status/expires · Expire-now quick action · joins users + campaigns. |
| `/invite/[token]` | `app/invite/[token]/page.tsx` | (root shell · no admin layout) | **Public · NOT operator-gated** — invitation accept flow (Phase 2.D.5). Token uuid is the bearer credential. Renders institutional landing card with campaign + tier + sender + expires; idempotent `opened` stamp on first visit; one-click Accept when signed in (calls `acceptInvitationAction` which links contact ↔ user, bootstraps subscription, preserves campaign attribution, writes audit). Blocking states (revoked/declined/bounced/expired/already-accepted) render their own institutional cards. |
