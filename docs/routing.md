...
| `/user/admin/users` | `app/user/admin/users/page.tsx` | `AdminLayout` | Real platform users — second stage of the conversion arc. 11 KPI totems (state + plan breakdowns) + 11-column table (User · Company/Org · Role · Linked contact · Status · Tier · Subscription · Promo · Last seen · Created · Owner) · URL-driven status + plan + linked-only + sort filters · joins `users` ↔ `organizations` ↔ `relationship_contacts` ↔ `subscriptions`. |
