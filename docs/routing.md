...
| `/user/admin/campaigns` | `app/user/admin/campaigns/page.tsx` | `AdminLayout` | Activation Layer (scaffold) — third stage. Foundation totems (campaigns defined · pending invitations · in-flight) + 7 planned campaign kinds enumerated. Full CRUD + Resend execution in Phase 2.D.4. |
| `/user/admin/subscriptions` | `app/user/admin/subscriptions/page.tsx` | `AdminLayout` | Monetization Layer (scaffold) — terminal stage. Status totems (total · active · trialing · past_due · canceled) + 5 tier ladder rows from `user_tier` enum. Management UI (grant Comped · mark Expired · refunds · per-org billing) in Phase 2.D.4. |
