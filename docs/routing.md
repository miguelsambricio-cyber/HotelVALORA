...
| `/user/admin/contacts` | `app/user/admin/contacts/page.tsx` | `AdminLayout` | Institutional Relationship Console — 14 KPI totems + 10-column institutional table (Contact · Company · Type · Band · Strength · Collab · Last email · Gmail labels · Email health · Strategic signal) · URL-driven band + investor-type + quality + sort filters · 50/page server pagination · reads from `relationship_contacts` + labels join. `?selected=<contact_id>` opens a server-rendered side drawer (Header · Institutional context · Strategic · Timeline · Peers — read-only). |
