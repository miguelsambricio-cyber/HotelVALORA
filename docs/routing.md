...
| `/user/admin/campaigns` | `app/user/admin/campaigns/page.tsx` | `AdminLayout` | Activation Layer (Live · Phase 2.D.4) — full CRUD: create/edit/archive/restore. KPI strip (5 status totems + invitation flow + conversion rate %) · filters (status/kind/archived/search/sort) · table (name+slug · kind · status · owner · active/converted/failed invitations · attributed subs · created). Form drawer doubles as Create (`selected=new`) and Edit (`selected=<id>`) with archive/restore quick action + last-25 invitation list. |
| `/user/admin/subscriptions` | `app/user/admin/subscriptions/page.tsx` | `AdminLayout` | Monetization Layer (Live · Phase 2.D.4) — full admin: Assign tier · Update tier/status/expires · Expire-now quick action. KPI strip (6 status totems + 7-tier breakdown) · filters (status/tier/attributed-to-campaign/sort/search) · table joins users + campaigns. Stripe-backed rows show an amber warning; comped/manual rows are fully operator-managed. |
