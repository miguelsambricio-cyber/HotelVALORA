...
| **Market warehouse vs underwriting operations split** — formal architectural separation. COMPSET moved out of `services/costar/`; CLASS added in its place (chain-scale aggregates at country or market level). New operational workspace at `services/compset/` with two masters (`COMPSET_MASTER.xlsx` 48c · `HOTEL_POSITIONING_MASTER.xlsx` 55c). Two new agents formally registered: **CoStar Market Data Agent** (`costar_market_data`, Tier 1) owns the slow-cadence warehouse; **CompSet Underwriting Agent** (`compset_underwriting`, Tier 2) owns on-demand per-hotel benchmark + positioning. DB migrations `market_vs_underwriting_split_enum_extend` + `market_vs_underwriting_split_seed_agents` applied — `ai_agent_id` enum extended, 2 registry rows seeded with full charters in jsonb. 4 new docs: `docs/architecture/market-vs-underwriting-separation.md` + `docs/agents/{costar-market-data-agent,compset-underwriting-agent,ceo-agent-supervision-layer}.md`. CEO Agent charter expanded for the new two-workspace supervision model (hourly health, cross-agent coordination, market-freshness + snapshot-freshness escalations). Costar workspace bumped to normalisation v1.1; new schema doc `costar-class-schema.md`; compset schema doc renamed `costar-compset-schema.md` → `compset-schema.md` + new `hotel-positioning-schema.md`. | *(this commit)* | ✅ |
...
3. **Set `INGESTION_AUDIT_TOKEN` on Vercel production + locally** — without it the audit-sync soft-fails every CLI run and `public.ai_agent_runs` does not reflect operator-side ingestions. `openssl rand -hex 32 | vercel env add INGESTION_AUDIT_TOKEN production`. Mirror to your shell's env.
4. **Phase 2.3.d.1 — CoStar Market Data Agent CLI** — mirror `services/transactions/scripts/ingest.py` for the four CoStar granularities (PAIS / MERCADO / SUBMERCADO / CLASS). Reuses `audit_sync.py` + cloud endpoint. New: per-granularity router, source readers tuned for CoStar/STR XLSX layouts, FX review flag, restatement detection. Activation flips `costar_market_data` → `beta`.
5. **Phase 2.4.1 — CompSet Underwriting Agent implementation** — TS agent + cloud route + operator CLI for `services/compset/`. Cross-workspace reader: pulls market context from `services/costar/MASTER/*` + subject metadata from `public.valuations`. Writes COMPSET row + HOTEL_POSITIONING snapshot per request. Activation flips `compset_underwriting` → `beta`.
4. **Phase 2 unit + integration tests** — regex categoriser fixtures, URL canonicaliser edge cases, RSS parser against captured payloads. Deferred from Phase 2 ship; lands as `0a` follow-up.
