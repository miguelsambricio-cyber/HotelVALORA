...
3. **Phase 2.3.b — Data Ingestion Agent supervises `services/transactions/`** — sweep INPUT_* (XLSX/CSV parser), normalise per `data-normalization-rules.md`, dedup against MASTER, append, route to staging on failure/review, archive on success, write INGESTION_LOG row, emit `data_ingestion_staged`. Operator trigger first; cron in Phase 2.4.
