...
4. **Phase 2.3.d.1 — CoStar CLI pipeline** — mirror the `services/transactions/scripts/ingest.py` shape for the four CoStar granularities. Reuses the existing `audit_sync.py` + cloud endpoint. New: per-granularity router, source-format readers tuned for CoStar/STR XLSX layouts, FX-conversion review flag, restatement detection (`compset_composition_change` review reason).
