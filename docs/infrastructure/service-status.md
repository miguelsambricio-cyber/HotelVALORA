...
| **Institutional transactions + projects workspace** | `services/transactions/` scaffolded — MASTER xlsx (transactions 59c, projects 50c) generated by `scripts/build_masters.py`. INPUT_*/old.*/staging/templates/logs directory tree with .gitignore tracking the contract (not the data). 5 architecture docs in `docs/intelligence/` (workflow, master-dataset, normalization, schemas). |
| **Data Ingestion Agent — operator pipeline (Phase 2.3.b)** | Python CLI at `services/transactions/scripts/ingest.py` — sweeps INPUT_*, parses XLSX/CSV with lenient header aliases, normalises (geography/dates/prices/entities/URLs/enums), dedups via sha256 against MASTER, routes valid rows to MASTER and borderline/broken rows to `staging/{review,failed}/`. Archives processed sources to `old.*/`. Writes per-run jsonl + MASTER INGESTION_LOG row. Smoke-tested end-to-end. Modules: dedup · normalization · master_io · staging_io · source_readers · ingest. |
| **Data Ingestion audit-chain unification (Phase 2.3.c)** | Cloud endpoint `POST /api/agents/data-ingestion-summary` (Bearer `INGESTION_AUDIT_TOKEN`) records one `ai_agent_runs` row per file with `metadata.python_ingestion_id` cross-reference + emits `data_ingestion_staged` event. Python CLI calls it after every successful run via `audit_sync.py`. Soft-fail philosophy: cloud unreachable → local run still authoritative. `public.ai_agent_runs` is now the single audit lens across CLI + cloud halves. Operator action: set `INGESTION_AUDIT_TOKEN` on Vercel + locally. |
| **Institutional CoStar warehouse (Phase 2.3.d.0 · v1.1)** | `services/costar/` scaffolded — 4 canonical MASTERs (PAIS 39c · MERCADOS 40c · SUBMERCADOS 41c · CLASS 41c) generated by `scripts/build_masters.py`. Four parallel pipelines (country/market/submarket/class) sharing the same 14-col ingestion-meta block + 5-sheet layout as `transactions/`. Owned by **CoStar Market Data Agent** (`costar_market_data`, planned). v1.1 split COMPSET out to a separate operational workspace. CLI pipeline lands in Phase 2.3.d.1. |
| **Operational compset workspace (Phase 2.4.0)** | `services/compset/` scaffolded — 2 canonical MASTERs (COMPSET_MASTER 48c · HOTEL_POSITIONING_MASTER 55c) generated by `scripts/build_masters.py`. Single INPUT/ + agent-routes-on-signature pattern. Owned by **CompSet Underwriting Agent** (`compset_underwriting`, planned, Tier 2). On-demand operational rhythm vs the costar warehouse's slow batch cadence — see `docs/architecture/market-vs-underwriting-separation.md`. Agent implementation lands in Phase 2.4.1. |
...
| Service | Phase |
|---|---|
| **CoStar Market Data Agent** — Python CLI ingest.py for the 4 warehouse granularities | Phase 2.3.d.1 — workspace substrate live, CLI is next deliverable |
| **CompSet Underwriting Agent** — TS agent + cloud route + operator CLI; reads warehouse, writes compset + positioning | Phase 2.4.1 — workspace substrate live in this commit |
...
**36 🟢 · 3 🟡 · 3 🔴 · 0 ⚫ across 42 active services**

Weighted score: (36 × 1.0 + 3 × 0.5 + 3 × 0.0) / 42 = **89%**. This commit ships the market-warehouse vs underwriting-operations split: COMPSET moved out of `services/costar/`, CLASS added in its place (v1.1); new operational workspace `services/compset/` (COMPSET_MASTER + HOTEL_POSITIONING_MASTER); two new agents registered (CoStar Market Data Agent + CompSet Underwriting Agent), both `planned` until their Phase 2.x implementations land; CEO Agent charter expanded for two-workspace supervision. 12 agents in registry · 3 in beta · 9 planned. Next-sprint candidates: set `INGESTION_AUDIT_TOKEN` on Vercel · Phase 2.3.d.1 CoStar Market Data Agent CLI · Phase 2.4.1 CompSet Underwriting Agent implementation · Phase 3 (pgvector + reactive orchestrator + CEO Agent activation).
