...
- **Supabase is live in production** — project `twebgqutuqgonabvhzjk` (eu-central · Postgres 17). 32-table schema + 5 Storage buckets + Library seed applied (migrations `0001`–`0005`); every table and `storage.objects` namespace has RLS; env wired on Vercel. `apps/web/src/lib/supabase/types.ts` regenerated from the live schema; typed Storage helpers ship at `lib/supabase/storage.ts` (browser) + `lib/supabase/storage-server.ts` (signed URLs). **Library surfaces are production-backed** — all four Library routes read from `public.valuations` (+ `top_promote_reports`, `favorite_reports`) via TanStack Query hooks (`lib/library/queries/*`) with optimistic favourite toggle. Supabase Auth still outstanding — Auth.js v5 owns identity today and the Zustand mock at `lib/auth/store.ts` is the only persisted client store left.
