...
- **Supabase is live in production** — project `twebgqutuqgonabvhzjk` (eu-central · Postgres 17). 41-table schema + 5 Storage buckets + Library seed + Intelligence Engine foundation applied (migrations `0001`–`0006`); every table and `storage.objects` namespace has RLS; env wired on Vercel. **Library surfaces are production-backed** via TanStack Query hooks. **Supabase Auth is wired and operational** (Google OAuth via Supabase Dashboard · `app/auth/callback/route.ts` exchanges code for HttpOnly cookies · `useAuth()` dual-source via `NEXT_PUBLIC_AUTH_ENABLED`). **Platform is in Public Beta / Showcase Mode** — `PROTECTED_PREFIXES` in `apps/web/src/middleware.ts` is intentionally empty so anonymous visitors can navigate every surface. **GitHub → Vercel auto-deploy is enabled** — push to `main` auto-deploys to production, push to any other branch auto-deploys to a preview URL. **Hospitality Intelligence Engine foundation in place** — 9 new tables (`market_news`, `hotel_transactions`, `hotel_projects`, `investors`, `operators`, `sources`, `news_entities`, `news_tags`, `news_ingestion_runs`) + 10 seeded sources; ingestion pipeline + Vercel Cron lands in Phase 2 (see `docs/intelligence/HOTELVALORA_HOSPITALITY_INTELLIGENCE_MASTER_SYSTEM.md`).
