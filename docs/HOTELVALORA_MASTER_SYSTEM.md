...
- **Supabase is live in production** — project `twebgqutuqgonabvhzjk` (eu-central · Postgres 17). 48-table schema + 5 Storage buckets + Library seed + Intelligence Engine + AI Operations Layer foundations applied (migrations `0001`–`0007`); every table and `storage.objects` namespace has RLS; env wired on Vercel. **Library surfaces are production-backed**. **Supabase Auth wired**. **Public Beta / Showcase Mode** (no route protection during validation). **GitHub → Vercel auto-deploy enabled**. **Hospitality Intelligence Engine foundation in place** — `docs/intelligence/`. **AI Operations Layer foundation in place** — 7 new tables, 9 operational AI systems declared (CFO · CMO · Customer Success · Underwriting · Market Intelligence · Data Ingestion · Report Generation · CRM/Dealflow · QA/Monitoring), 20 tools catalogued, full strategic + technical docs at `docs/ai-agents/AI_OPERATIONS_LAYER_MASTER_SYSTEM.md`. **These are NOT chatbots and NOT a side feature — they are a future core operating layer.** Phase 2+ implements agent runtimes; Tier 1 agents (Market Intelligence + Data Ingestion + QA / Monitoring) are the next-sprint candidate.
