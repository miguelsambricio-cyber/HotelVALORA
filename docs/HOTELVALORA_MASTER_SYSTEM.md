...
> **2026-05-20 · Phase 4 wave 2 · Competitive Set + Market Overview canonical + admin↔reports bridge.** 4 of 7 report sections now canonical-backed. `/user/admin/hotels/[hotelId]` sidebar surfaces "View as report" group with 4 links (Executive Summary · Asset Analysis · Competitive Set · Market Overview) above "Open underwriting view" · only renders when canonical_id resolves · amber callout otherwise pointing to the correction queue. Competitive Set: subject from canonical + 4 nearest peers via chain_scale + haversine. Market Overview: hotelLabel + numeric KPI overrides from snapshot timeseries · narrative content preserved from mock for now. Full 253-corpus sync audit at `docs/hotel-intelligence/corpus-sync-audit-2026-05-20.md` — 224 canonical hotels (88% of 253 target) · 100% market+submarket+geo+hero · 93-97% phone/website/place_id · structural blockers `total_rooms`/`year_opened` 0% (D-8 gated). Phase 4 operational TODAY for 112 branded Madrid hotels across 4 of 7 sections. Hard rule respected — zero UI/shell/primitive/PDF touches.

> **2026-05-20 · Phase 4 mock → canonical migration · Executive Summary + Asset Analysis live.** Reports now consume `public.hotel_canonical` via new data layer (`apps/web/src/lib/report/canonical-reader.ts` + per-section mappers under `canonical-mappers/`). Pages wired with `?canonical_id=<uuid>` AND `?hotel_id=h_<hex>` resolution (3-path multi-resolver mirrors admin detail logic). Mock fallback preserved when no canonical_id is supplied. Market KPIs (ADR/occupancy/RevPAR/cap-rate/per-room) read from snapshot timeseries (real CoStar warehouse data). Valuation block · capRate + perRoom real · GOP margin + EBITDA + per-sqm stubs (placeholder until cap-rate engine wires `runForHotel`). Asset Analysis · facilities from `amenities` JSONB · room mix derived from total_keys × chain_scale heuristic until D-8 fills `room_type_mix`. Smoke target: Mandarin Oriental Ritz (`/report/executive-summary?canonical_id=dafc4073-…`). Hard rule respected — zero UI/shell/primitive/PDF changes; only data layer + page-level dispatchers. Operator milestone unlocked: admin edits → Supabase → next report render reads latest, no more isolated mock data for ExecSummary + Asset Analysis paths.

> **2026-05-20 · End-to-end smoke test (Mandarin Oriental Ritz) · admin↔reports linkage validated.** Subject: Mandarin Oriental Ritz Madrid (`dafc4073-…`). Findings: canonical layer ✅ (all admin fields populate) · provenance ✅ (5176 field rows · 9 dedup marks) · admin loop ✅ (drawer persists to Supabase · overlay surfaces edits immediately) · **reports ← canonical NOT WIRED** (all 7 sections read `getMock*()` hardcoded demo data · zero canonical reads). 3-source consumer architecture required: `getHotelCanonical(id)` + `getMarketKPIs(market, submarket)` + `runUnderwriting(id)`. Only #1 has Supabase implementation today. Phase 4 mock→canonical migration plan laid out in §7-§8 of the smoke test doc (`docs/hotel-intelligence/smoke-test-mandarin-oriental-ritz-2026-05-20.md`). Inventory milestone status: admin loop COMPLETE · report loop BLOCKED on Phase 4. Hard rule respected — no `/report/*` UI touches.
