...
/**
 * LEGACY BRIDGE · /report/financials/pl.
...
export default async function LegacyPLPage({ searchParams = {} }: PageProps) {
  const legacyInput = searchParams?.canonical_id ?? searchParams?.hotel_id ?? searchParams?.ref;
  if (legacyInput) {
    const session = await createOrGetReport({
      input: legacyInput,
      inputParams: { legacy_input: legacyInput, section: "financials/pl" },
    });
    if (session) redirect(`/report/${session.report_id}/financials/pl`);
  }
  redirect("/report/legacy-mock/financials/pl");
}
