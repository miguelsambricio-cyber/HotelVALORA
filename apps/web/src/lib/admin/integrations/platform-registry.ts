...
const MAPS_GEO_INTELLIGENCE: PlatformIntegrationDescriptor[] = [
  {
    ...
    status: "live",
    purpose:
      "Tourist demand heatmaps and urban connectivity overlays. Worldwide popularity scores derived from 60+ public data sources · 2M activity signals/hour · used by Booking · Kayak · Priceline · Sonder.",
    authMethod: "Public scriptId · domain-restricted in the AVUXI dashboard (hotelvalora.com authorised)",
    envVars: ["NEXT_PUBLIC_AVUXI_ENABLED"],
    tables: [],
    cronDependencies: [],
    consumedBy: [
      "/compset (when NEXT_PUBLIC_AVUXI_ENABLED=true · skips manual heatmap+metro · drives via CAPAS panel)",
      "/report/competitive-set · /report/executive-summary · /report/market-overview (via ReportMap · same flag)",
      "/experiment-avuxi (validation surface · v9 baseline · always-on regardless of flag)",
      "/experiment-avuxi-sandbox (raw HTML/JS reference test)",
    ],
    operatorManaged: false,
    signal: "ok",
    accountProvisioned: true,
    nextMilestone:
      "Preview deployment with NEXT_PUBLIC_AVUXI_ENABLED=true · validate 10-item pre-deploy checklist · then flip in production env.",
    notes: [
      "Feature-flagged deployment (Phase 2 · 2026-05-22) · NEXT_PUBLIC_AVUXI_ENABLED=true activates AVUXI · false (default) falls back to HotelValora native MapHeatmapLayer + MapMetroLayer + MapPolygonLayer.",
      "CAPAS panel is the único control surface · AVUXI native UI hidden via single CSS rule (.category-control-container · display:none).",
      "Coverage: current licensed coverage · heatmaps + POIs worldwide (algorithm-driven) · 70+ cities for transit (curated).",
      "Product: Map Layers for Mapbox · scriptId fad4d930-e615-4c0c-9d15-e5f8fdd2224a (operator-provisioned · institutional · NOT the demo from documentation).",
      "SDK signature: AVUXI.mapStart(map, mapboxglNamespace, scriptId, options) · 4 args.",
      "Centro Histórico (MapPolygonLayer · HISTORIC_CENTER_POLYGON) is HV-native · independent of AVUXI · unaffected by feature flag.",
      "Rollback: flip NEXT_PUBLIC_AVUXI_ENABLED=false on Vercel · ~3-min redeploy · manual layers re-render from lib/maps/geo-data.ts (preserved as fallback per operator constraint).",
      "Pricing: free tier ≤ 1000 widget loads/month · paid tier negotiated with AVUXI sales pre-public-launch.",
    ...
    externalLinks: [
      ...
      { label: "Phase 2 execution plan", href: "https://github.com/miguelsambricio-cyber/HotelVALORA/blob/main/docs/maps/avuxi-phase-2-execution-plan.md" },
      { label: "Phase 2 final mockup", href: "https://github.com/miguelsambricio-cyber/HotelVALORA/blob/main/docs/maps/avuxi-phase-2-mockup.md" },
      { label: "Integration architecture", href: "https://github.com/miguelsambricio-cyber/HotelVALORA/blob/main/docs/maps/avuxi-integration-architecture.md" },
    ],
  },
];
